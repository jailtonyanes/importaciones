                                ...
                                <div class="row" style="margin-left:28%"> 
                                  <div class="col-md-12">       
                                            <div class="form-group">
                                                <label for="provincia">Seleccione cotización:</label>
                                                <br/>
...
FROM
  `cliente`
    ...
    INNER JOIN `items` 
        ON (`cotizacion_id` = `cotizacion`.`id`) where cotizacion.estado ='COTIZADO' GROUP BY cotizacion.id ORDER BY cotizacion.id");
                                                         $datos1 = $crud->seleccionar($con->getConection());
                                                         $i=0;
                                                         while ($i<sizeof($datos1))
                                                         {   
                                                     ?>
                                                          <option value="<?php echo $datos1[$i]['cotizacion']?>"><?php echo 'Cot_nro: '.$datos1[$i]['cotizacion'].'**'.'SMI-'.$datos1[$i]['cliente'].'**'.$datos1[$i]['nombre'].' '.$datos1[$i]['apellido'].'**'.$datos1[$i]['fecha_creacion'] ?></option>
                                                     <?php
                                                           $i++;
                                                          $con->desconectar();
                                                         } 
               ...
               -->          
                          ...
                          <div class="box-footer">
                           ...
                           <div class="row" style="padding-left:2%">   
                                      <div class"col-md-6">
                                              Subtotal: &nbsp;
                            <input type="text" style="width:82px" id="sub_t_fact" name="sub_t_fact"  class="currency" readonly placeholder="Subtotal">
                            &nbsp;&nbsp;ITBMS:&nbsp;
                            <input type="text" style="width:82px" id="itbms_t" name="itbms_t" class="currency"  readonly placeholder="ITBMS">
                              &nbsp;&nbsp;NETO A PAGAR:&nbsp;
                            <input type="text" style="width:82px"  id="neto_pg" name="neto_pg"  class="currency" readonly placeholder="Neto a pagar">
                              &nbsp;&nbsp;% DE ABONO:&nbsp;
                            <select name="porc_ab" id="porc_ab" onchange="porc_abono()" >
                              ...
                              <option value="100">100</option>
                               ...
                               &nbsp;&nbsp;CANTIDAD A ABONAR:&nbsp;
                            <input type="text" style="width:82px"  id="c_abonar" name="c_abonar"  class="currency" readonly >
                              RESTANTE:&nbsp;
                            <input type="text" style="width:82px"  id="restante" name="restante"  class="currency" readonly >
                              ...
                              <br/>
                              <div class="row" >
                                       <div class="col-md-6 text-right" style="margin-left:48%"> 

                                       <input type="button"  name="g_coti" id="g_coti" onclick="save_coti()"  class="btn btn-success" value="Guardar Borrador">
                                        <input type="button"  name="g_coti" id="g_coti" onclick="cambia_estado()"  class="btn btn-warning" value="Cambiar a pedido">
...
<script type="text/javascript">
          function cambia_estado()
          {
                      ...
                      if(confirm('Desea cambiar el estado de la cotizaci\u00f3n a estado pedido? '))
                      { 

                        $.post('../../admin/cambia_estado.php',{
                          ...
                          function(data){
                                 ...
                                 if(jQuery.trim(data)=='Estado cambiado con exito.')
                                 {
                                   location.reload(); 
                                     //
                                 }
...
<script type="text/javascript">
    function porc_abono()
    {
       var tot_pagar =parseFloat(moneda_dec($("#neto_pg").val()));
       var por_abono =parseFloat(moneda_dec($("#porc_ab").val()));
       var abono = tot_pagar * por_abono /100;
       $("#c_abonar").val(Math.round(abono*100)/100);

       var restante2 = tot_pagar - parseFloat($("#c_abonar").val());
       $("#restante").val(Math.round(restante2*100)/100);
       ...
       function sacar_info_cli()
        {
           ...
           $.get('../../admin/xml_clientes_id.php?ocul='+user,{
            type: 'xml'
          },
          function(xml){
           ...
           $("#refe").val($(xml).find('cot_id').text());
           $("#fech").val($(xml).find('fecha_creacion').text());
           $("#contador").val($(xml).find('contador').text());
            $("#sub_t_fact").val($(xml).find('total').text());
             $("#itbms_t").val($(xml).find('f_smi').text());
           $("#neto_pg").val($(xml).find('t_pagar').text());
          ...
          // $("#ingreso_pac").val('Actualizar');
           ...
           //sacar el contenido de las grillas

                 $("#cont_list").load("../../admin/saca_items.php?uid="+user,function(){
                  $('#table_id').DataTable();
                  $(this).fadeIn("medium");
                  });
           //fin---
...
//elementos de la calculadora
            ...
            function save_coti()
            {
                   ...
                   if(confirm('Desea guardar la cotizaci\u00f3n en borrador?'))
                   {
                         if($("#refe").val()=='')
                         {
                                  ...
                                  function(data)
                                   {
                                     ...
                                     if(jQuery.trim(data)=='Datos guardados con exito.')
                                     {
                                           ...
                                           $('#ocul_descr_1').val('');
                                           $('#ocul_link_1').val('');
                                            $('#tipo_serv_1').val('Escoja opción');
                                           $('#precio_1').val('');
                                           $('#cantidad_1').val('');
                                           $('#costo_ship_1').val('');
                                           $('#peso_1').val('');
                                           $('#state_tax_1').val('Escoja opción');
                                           $('#pa_1').val('');
                                           $('#flete_1').val('');
                                           $('#subtotal_1').val('');
